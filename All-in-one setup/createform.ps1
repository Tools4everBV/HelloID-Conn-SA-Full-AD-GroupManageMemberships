...
'@;
        ...
        Write-ColorOutput Green "Powershell task '$taskName' created: $taskGetADGroupMembershipsGuid"  
  
    } else {
        #Get TaskGUID
        $taskGetADGroupMembershipsGuid = $response.automationTaskGuid
        Write-ColorOutput Yellow "Powershell task '$taskName' already exists: $taskGetADGroupMembershipsGuid"
    }
} catch {
    Write-ColorOutput Red "Powershell task '$taskName'"
    $_
...
try {
    ...
    if([string]::IsNullOrEmpty($response.dataSourceGUID)) {
        ...
        Write-ColorOutput Green "Task data source '$dataSourceName' created: $dataSourceGetADGroupMembershipsGuid"
    } else {
        #Get DatasourceGUID
        $dataSourceGetADGroupMembershipsGuid = $response.dataSourceGUID
        Write-ColorOutput Yellow "Task data source '$dataSourceName' already exists: $dataSourceGetADGroupMembershipsGuid"
    }
} catch {
    Write-ColorOutput Red "Task data source '$dataSourceName'"
    $_
